...
The dashboard is a local operator view for the simulator. It provides start, stop, reset, polling, severity filters, a MITRE-grouped timeline, an alert panel, and a threat analytics view.

![Dashboard attack timeline](docs/screenshots/dashboard-attack-timeline.png)

Completed local simulation showing the Key Vault, ARM, and Storage steps. These are simulator events, not direct Log Analytics records.

![MITRE threat map](docs/screenshots/dashboard-mitre-threat-map.png)

The Threat Analytics view groups local simulator events by MITRE ATT&CK tactic and technique.
...
Azure Monitor alert email showing that the Key Vault detection crossed its configured threshold and fired the scheduled query alert.
